...
object ListApi {
  ...
  def main(args: Array[String]) {
    ...
    println("Initialize 'thrill' List : " + thrill)
    ...
    println("Return 'thrill' satisfying the condition : " + thrill.filter(s => s.length == 4))
    ...
    // 要素数を返す
    println("Return the number of elements in 'thrill' : " + thrill.length)

    // リストの各要素に引数（関数）を適用したListを返す
    println("Return the List which the processing to each element of 'thrill' : " + thrill.map(s => s + "y"))

    // リストの要素を並べた文字列を返す
    println("Return a string that lined element of 'thrill' : " + thrill.mkString(", "))
